...
def compute_distrib_percentile(read_directory_historical,read_directory_rcp,write_directory,start_year_ref=1975,end_year_ref=2025,temp_variable='tasmax',threshold_value=95,distrib_window_size=15,anomaly=True) :
    ...
    for day in tqdm(range(1,366)) : # Calendar days ranging from 1 to 365 (no leap years)
        if day - (distrib_window_size//2) <= 0  : # day is at the beginning of January, window overlapping with December
            distrib_start_bound = 365 + day - (distrib_window_size//2) # start boundary of the distribution window
...
def create_heatwaves_indices_database(read_directory_historical,read_directory_rcp,write_directory,pop_data_path,other_data_path,start_year=1975,end_year=2099,start_year_ref=1975,end_year_ref=2025,temp_variable='tasmax',threshold_value=95,anomaly=True):
    ...
    for year in tqdm(range(start_year,end_year+1)): # Iterate over the years
        ...
        for label in labels_list : # Iterate over the heatwaves (one label = one heatwave).
            ...
            if year<=2030: #GHS-POP covers 1975-2030
                #TODO compute ghs population
                #TODO Compute correct population matching ssp
